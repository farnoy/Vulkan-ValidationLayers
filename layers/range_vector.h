  ...
  private:
    ...
    SmallRange clear_out_range(const SmallRange &clear_range, bool valid_clear_range) {
        ...
        if (first_range.valid() && first_range.includes(clear_range)) {
            ...
            auto last_included = clear_range.end - 1;
            RANGE_ASSERT(last_included < limit_);
            RANGE_ASSERT(!first_range.includes(last_included));  // again, this should only be true in the "included" case above
            const auto &last_range = ranges_[last_included];
            if (last_range.valid()) {
                // Trim right
        ...
        if (valid_clear_range) {
            ...
            rerange_end(empty_right.begin, empty_left.end, clear_range.end);
        } else {
